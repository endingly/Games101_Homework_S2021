                                         ...
                                         const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    // 包围盒边界
    auto bound1_min = std::min(std::min(v[0].x(), v[1].x()), v[2].x());
    auto bound2_min = std::min(std::min(v[0].y(), v[1].y()), v[2].y());
    auto bound1_max = std::max(std::max(v[0].x(), v[1].x()), v[2].x());
    auto bound2_max = std::max(std::max(v[0].y(), v[1].y()), v[2].y());

    // 渲染循环
    for (int x = bound1_min; x <= bound1_max; x++)
    {
        for (int y = bound2_min; y <= bound2_max; y++)
        {
            // 保证在三角形中
